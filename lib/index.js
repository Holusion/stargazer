...
const {dispatchList, listenTasks, dispatchTask, listenError, listenInfo, dispatchError, endTask} = require("./store");
...
function updateProductList(front=false){
  if(!updating){
...
(function() {
  ...
  ipcRenderer.on("clients-list",function(evt, message){
    ...
    if (Array.isArray(message)){
      ...
      notifier.pushError(new Error("clients-list is not an array : "+ JSON.stringify(message)));
    }
  });
