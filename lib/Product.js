...
class Product extends HTMLElement{
  ...
  static render(elem, list){
    ...
    <h-playlist url="${infos.addresses[0]}" slot="content"/>
    `;
  }
}
...
class Playlist extends HTMLElement {
  ...
  absoluteUrl(path){
    //FIXME use url.format()
    return url.resolve("http://"+this.getAttribute("url"),path);
  }
  async update(){
    let r;
    ...
    let target = this.absoluteUrl("/playlist");
  ...
  async requestUpdate(method, targetPath, body){
    let target = this.absoluteUrl(targetPath);
    let opts = {};
