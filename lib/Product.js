...
class Playlist extends HTMLElement {
  ...
  connectedCallback(){
    ...
    fetch(options).then(e => {
      this.current = JSON.parse(e).name;
      this.update(url);
    });
  }
  ...
  updateToolbar(elem) {
    let toolbar = document.querySelector(".mdc-toolbar__section--align-end");
    while(toolbar.firstChild) {
      toolbar.removeChild(toolbar.firstChild);
    }
    if(document.querySelectorAll('.is-selected').length == 1) {
      let playButton = Object.assign(document.createElement("button"), {
          className: "play-menu material-icons mdc-toolbar__menu-icon mdc-ripple-upgraded--foreground-activation",
  ...
  render(list){
    ...
    list.map((l)=>{delete l._id; return l;}).map((info)=>{
      ...
      let self = this;
      const menu = new Menu();
      menu.append(new MenuItem({label: "Jouer", click() {if(info.active) self.requestUpdate("put", `/control/current/${info.name}`)}}));
      ...
      menu.append(new MenuItem({label: "Supprimer", click() {self.requestUpdate("delete", `/medias/${info.name}`, null)}}))
