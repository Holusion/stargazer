...
class ProductPanel extends HTMLElement {
  ...
  async listenChange(evt) {
    ...
    switch(evt.detail.action) {
      case 'selected': 
        this._listenSelected(elem);
        break;
      case 'start':
        await this._listenStart(elem);
        break;
      case 'removeitem':
        this._listenRemoveItem(elem, removePopup);
        break;
      case 'visible':
        await this._listenVisible(elem);
        break;
      case 'active':
        await this._listenActive(evt, elem);
        break;
    }
  }

  _listenSelected(elem) {
    if(elem.hasAttribute('selected')){
      try {
        this.playlist.selectItem(elem);
      } catch(err) {
        dispatchError(new ErrorWrapper("selectedItem()", err));
      }
    } else {
      try {
        this.playlist.unselectItem(elem);
      } catch(err) {
        dispatchError(new ErrorWrapper("unselectedItem()", err));
      }
    }
    this.updateToolbar();
  ...
  _listenRemoveItem(elem, removePopup) {
  ...
  render(list){
    ...
    if(this.playlist != []) {
      ...
      buttonSelectAll.predicate = () => this.playlist.medias.filter(elem => elem.selected).length === 0;
