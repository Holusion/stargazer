...
class ProductPanel extends HTMLElement {
  ...
  render(list){
    ...
    if(this.playlist != []) {
      ...
      let spanSelectAll = document.createElement('span');
      selectAll.classList = "mdc-button mdc-button--raised";
      selectAll.addEventListener('click', () => {
        let items = document.querySelectorAll("h-item");
        items.forEach(e => e.selected = true);
      })
      spanSelectAll.classList = ["mdc-button__label"];
      spanSelectAll.textContent = "Tout sélectionner";
      Object.assign(selectAll.style, {'margin-bottom': '16px'})
      selectAll.appendChild(spanSelectAll);
      surroundD.appendChild(selectAll)
